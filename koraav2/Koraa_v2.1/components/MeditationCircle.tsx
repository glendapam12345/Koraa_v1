...
export function MeditationCircle({ visible, onComplete, onClose, type }: MeditationCircleProps) {
  ...
  const secScale = useSharedValue(1);
  const holdRingGlow = useSharedValue(0);
  ...
  useEffect(() => {
    ...
    if (prevSecRef.current !== null && prevSecRef.current !== secondsRemaining) {
      secScale.value = 1.13;
      secScale.value = withSpring(1, { damping: 14, stiffness: 220 });
    }
    prevSecRef.current = secondsRemaining;
  }, [secondsRemaining, isActive, secScale]);

  /** Halo cuando el trazo del círculo está lleno (aguanta) */
  useEffect(() => {
    if (breathPhase === 'hold' && isActive) {
      holdRingGlow.value = withRepeat(
        withSequence(
      ...
      holdRingGlow.value = withTiming(0, { duration: 220 });
    }
  }, [breathPhase, isActive, holdRingGlow]);
  ...
  const timerScaleStyle = useAnimatedStyle(() => ({
    transform: [{ scale: secScale.value }],
  }));

  const fullHaloStyle = useAnimatedStyle(() => ({
    opacity: 0.15 + holdRingGlow.value * 0.82,
    transform: [{ scale: 1 + holdRingGlow.value * 0.055 }],
  }));
        ...
        >
          ...
          <View style={styles.content}>
            ...
            <View style={styles.circleContainer}>
              <Animated.View
                pointerEvents="none"
                style={[styles.fullHaloRing, fullHaloStyle]}
              />
              {/* SVG Circle Progress */}
              <Svg
                width={CIRCLE_SIZE}
                ...
                {isActive ? (
                  <View
                    style={[
                      ...
                      breathPhase === 'hold' && styles.timerContainerFull,
                    ]}
                  >
