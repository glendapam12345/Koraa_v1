...
export function MeditationCircleSimple({ visible, onComplete, onClose, type }: MeditationCircleSimpleProps) {
  ...
  }, [visible, waveAnim, secScale, holdPulse]);
  ...
  /** Pulso suave en el número cada vez que cambia el segundo */
  useEffect(() => {
    if (!isActive || isCompleted) {
      prevSecRef.current = null;
      return;
    }
    if (prevSecRef.current !== null && prevSecRef.current !== secondsRemaining) {
      secScale.setValue(1.12);
      Animated.spring(secScale, {
        toValue: 1,
        friction: 7,
        tension: 160,
        useNativeDriver: true,
  ...
  }, [secondsRemaining, isActive, isCompleted, secScale]);

  /** Halo luminoso cuando el anillo está lleno (aguanta el aire) */
  useEffect(() => {
    if (!isActive || isCompleted || breathPhase !== 'hold') {
      holdPulse.setValue(0);
      return;
    }
    holdPulse.setValue(0.45);
    const loop = Animated.loop(
      Animated.sequence([
    ...
    loop.start();
    return () => loop.stop();
  }, [breathPhase, isActive, isCompleted, holdPulse]);

  const holdHaloOpacity = holdPulse.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 0.95],
  });
  const holdHaloScale = holdPulse.interpolate({
    inputRange: [0, 1],
    outputRange: [1, 1.06],
  });
            ...
            {isCompleted ? (
              ...
              <>
                ...
                <View style={[styles.circleWrapper, { width: CIRCLE_SIZE + 56, height: CIRCLE_SIZE + 56 }]}>
                  ...
                  {/* Halo al estar el círculo lleno (fase aguantar) */}
                  <Animated.View
                    pointerEvents="none"
                    style={[
                      styles.fullRingHalo,
                      {
                        ...
                        opacity: holdHaloOpacity,
                        transform: [{ scale: holdHaloScale }],
                      },
                    ]}
                  />
                  <View style={[styles.circleContainer, { width: CIRCLE_SIZE + 8, height: CIRCLE_SIZE + 8 }]}>
                    ...
                    <View style={styles.ringCenter}>
                      <View
                        style={[
                          styles.centerGlass,
                          breathPhase === 'hold' && isActive && styles.centerGlassFull,
                        ]}
                      >
                        {!isActive ? (
                          <Sparkles size={76} color={THEME.colors.fill[100]} strokeWidth={1.5} />
                        ) : (
                          <Animated.View style={[styles.timerBlock, { transform: [{ scale: secScale }] }]}>
...
const styles = StyleSheet.create({
  ...
  fullRingHalo: {
    position: 'absolute',
    borderWidth: 3,
    borderColor: THEME.colors.fill[100],
    backgroundColor: 'transparent',
    shadowColor: THEME.colors.fill[100],
    shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.65,
    shadowRadius: 24,
    elevation: 6,
