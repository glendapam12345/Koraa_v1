...
import { Alert, Platform, StyleSheet, Text, TouchableOpacity, ActivityIndicator } from 'react-native';
...
export function AddToDeviceCalendarButton({
  ...
  const showCalendarError = useCallback(
    (result: AddTaskToDeviceCalendarResult) => {
      if (result.ok) return;
      if (result.reason === 'native_module_missing') {
        Alert.alert(
          t('deviceCalendar.unavailableTitle'),
          t('deviceCalendar.unavailableBody'),
        );
        return;
      }
      if (result.reason === 'permission_denied') {
        Alert.alert(t('deviceCalendar.permissionTitle'), t('deviceCalendar.permissionBody'));
        return;
      }
  ...
  const handlePress = useCallback(async () => {
    ...
    try {
      ...
      if (result.reason === 'already_added') {
        Alert.alert(
          t('deviceCalendar.alreadyAddedTitle'),
          t('deviceCalendar.alreadyAddedBody'),
          [
  ...
  }, [loading, scheduledDate, showCalendarError, taskId, title, t]);
  ...
  return (
    <TouchableOpacity
      ...
      accessibilityLabel={t('deviceCalendar.a11y', { task: title.trim() || t('taskCard.taskFallback') })}
