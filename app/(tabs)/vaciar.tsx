...
import { hoyTaskIdsFromCaptureItems, type CaptureHoyEntry } from '@/lib/hoyTaskIdsFromCapture';
...
import { goToHoyTab } from '@/lib/vaciarNavigation';
...
function VaciarScreen() {
  ...
  const [fromHoy, setFromHoy] = useState(false);
  ...
  const finishCaptureAndLeave = useCallback(
    async (
      entries: CaptureHoyEntry[] = [],
      next: 'hoy' | 'tips' = 'hoy',
    ) => {
      const idsForHoy = hoyTaskIdsFromCaptureItems(entries);
      if (user?.id && idsForHoy.length > 0) {
        await promoteTaskIdsToHoy(user.id, idsForHoy);
      } else if (user?.id && entries.length === 0) {
        await ensureOneHoyStepFromCapture(user.id);
      }
      requestHoyRefresh(idsForHoy.length > 0 ? { pinTaskIds: idsForHoy } : undefined);
      if (next === 'tips') {
    ...
    [resetCaptureFlow, router, user?.id],
  );
  ...
  const handleRelease = useCallback(async (projectsForMatch: { id: string; name: string }[] = []) => {
    ...
    if (!useAdvanced) {
      let live: ReturnType<typeof buildLiveCapturePreview> = null;
      try {
        live = buildLiveCapturePreview(taskInput, locale, projectsForMatch, {
          lifeAreasConfig: effectiveLifeAreasConfig,
        });
      } catch (error) {
        logger.error('vaciar.livePreview', error);
      }
      ...
      if (fromHoy && live.items.length === 1) {
        ...
        try {
          ...
          const items = applyInferredLifeAreas(
            ...
            ...item,
            selectedDate: item.selectedDate ?? today,
            markImportant: index === 0,
          }));
          ...
          await finishCaptureAndLeave(
            batchResult.tasks.map((entry) => ({
              taskId: entry.taskId,
              selectedDate: today,
            })),
          );
        } catch (error) {
          logger.error('vaciar.hoyRelease', error);
          showToast(t('errors.saveTaskFailed'), 'error');
        } finally {
          setIsOrganizing(false);
        }
      ...
      void (async () => {
        try {
          ...
          const { items: refined } = await applyAiProjectHints(
            live.items,
            taskInput,
            locale,
            user.id,
  ...
  const handleConfirmPreview = useCallback(
    async (payload?: {
      ...
      try {
        ...
        let savedEntries: CaptureHoyEntry[] = [];
        ...
        if (items.length === 1 && hasSubtasks) {
            ...
            savedEntries = [
              {
                ...
                markImportant: Boolean(item.markImportant),
              },
            ];
          }
        } else {
          ...
          savedEntries = batchResult.tasks.map((entry) => {
            const item = items.find((row) => row.id === entry.captureId);
            return {
              taskId: entry.taskId,
              selectedDate: item?.selectedDate ?? null,
              markImportant: Boolean(item?.markImportant),
        ...
        setTaskInput('');
        resetTaskForm();
        await finishCaptureAndLeave(savedEntries, next);
      } catch (error) {
        logger.error('vaciar.confirmPreview', error);
        showToast(t('errors.saveTaskFailed'), 'error');
      } finally {
  ...
  const applyFreshCapture = useCallback(() => {
    const openedFromHoy = isHoyCaptureSource(sourceParam);
    resetCaptureFlow();
    setFromHoy(openedFromHoy);
    setSegment('capture');
  ...
  }, [dateParam, projectIdParam, resetCaptureFlow, sourceParam, suggestion]);
  ...
  useEffect(() => {
    ...
    if (!takeFreshCaptureStamp(stamp)) return;
  ...
  useFocusEffect(
    useCallback(() => {
      if (!isHoyCaptureSource(sourceParam)) {
        setFromHoy(false);
      }
      void refreshCheckInToday();
      void loadHintState();
    ...
    }, [loadProjectCount, refreshCheckInToday, loadHintState, refreshFocusedProject, sourceParam]),
  );
            ...
            >
              ...
              {isCaptureSegment && captureStep === 'organized' && user && savedOrganizedContext ? (
                <BrainDumpSavedSummaryScreen
                  ...
                  onGoToHoy={() => goToHoyTab()}
                  onGoToCheckIn={() => goToHoyTab()}
                  onGoToTips={() => openTipsCategory(router, 'mindset')}
